...
export function buildSystemPrompt(context: {
  ...
  const parts = [
    ...
    '',
    '## What the metrics cover and what they don\'t',
    '',
    '**Acceptance Rate** measures inline code completions only (ghost-text suggestions offered vs accepted in the editor).',
    'It does NOT include Chat interactions, Agent mode, Copilot CLI, or GitHub.com Copilot usage.',
    'Users who primarily use Chat/Agent mode will naturally have low acceptance rates — this is expected and does not indicate low Copilot value.',
    '',
    '**Per-user data** (from the users-28-day report) includes:',
    '- Active days, interaction counts, code generation & acceptance events, lines of code',
    '- Feature breakdown (totals_by_feature): code_completion, chat_panel_ask_mode, chat_panel_agent_mode, agent_edit, chat_inline',
    '- IDE breakdown and language breakdown',
    '- You CAN tell which users use Chat vs Agent vs code completion from the feature breakdown',
    '',
    '**NOT available per-user:**',
    '- Copilot CLI usage (only aggregate daily_active_cli_users at org level)',
    '- Detailed pull request stats per user (only aggregate PR metrics)',
    '- GitHub.com Copilot chat is partially captured under chat features',
    '',
    '**Agent activity** (aggregate only): PR creation/review/merge stats, Copilot-authored PRs, monthly active agent users.',
    'Per-user agent usage can be inferred from totals_by_feature containing chat_panel_agent_mode or agent_edit.',
    '',
    'When users ask about low acceptance rates or missing activity, explain these metric boundaries.',
    'For more details, refer users to: https://docs.github.com/en/copilot/rolling-out-github-copilot-at-scale/analyzing-usage-over-time',
  ];
