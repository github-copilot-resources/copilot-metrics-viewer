...
import { useRoute } from 'vue-router'; // Generated by Copilot: Import useRoute
...
const organization = computed(() => {
  // First check if organization is in route params
  if (route.params.org) {
...
// Generated by Copilot: Debug route and config information
console.log('Route params:', route.params);
console.log('Runtime config org:', config.public.githubOrg);
...
watch(organization, async (newOrg, oldOrg) => {
  if (newOrg && newOrg !== oldOrg) {
    ...
    selectedTeams.value = []; // Reset selected teams
    allTeams.value = []; // Clear all teams
    teamMetrics.value = []; // Clear team metrics data
    metrics.value = []; // Clear metrics data
...
const fetchAllTeams = async () => {
  try {
    ...
    // Generated by Copilot: Get the current organization
    const currentOrg = organization.value;
    if (!currentOrg) {
      console.warn('No organization specified');
      return;
    }
    
    console.log('Fetching teams for organization:', currentOrg);
    // Generated by Copilot: Pass organization as query parameter
...
const handleTeamsChange = async () => {
  try {
    ...
    // Generated by Copilot: Use current organization from route
    const currentOrg = organization.value;
    console.log(`Fetching metrics for teams in organization ${currentOrg}:`, teamsToFetch);
    
    const response = await $fetch('/api/teams-metrics', {
      method: 'POST',
      body: {
        teams: teamsToFetch,
        organization: currentOrg // Generated by Copilot: Send organization to API
      }
    });
